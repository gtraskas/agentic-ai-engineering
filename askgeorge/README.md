...
- **Any language:** questions are answered in the language they were asked in, and so are the refusals (rate limit, guardrail) that never reach the model, under a hard daily budget so refused traffic cannot become a cost centre of its own. Because the corpus and the embedding model are English-only, a non-English question is translated to English for the retrieval step before it is searched, so the answer stays grounded instead of fluent-but-thin. Questions that are already English skip the call, so the common path costs nothing
- **Report download:** a finished job-fit report can be downloaded as Markdown. Each report is written to its own temp directory, so concurrent visitors can never be served each other's analysis; directories older than an hour are pruned on each write so a long-lived container stays clean
